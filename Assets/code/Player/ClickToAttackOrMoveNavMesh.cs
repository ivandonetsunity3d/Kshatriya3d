...
public class ClickToAttackOrMoveNavMesh : MonoBehaviour {
    ...
    public void PlayerClicksOnGameObject(GameObject hittransformgameObject)
    {
        ...
        if (destinationDistance > 1f)
        {
            ...
            Approach(hittransformgameObject);
        ...
        else if (destinationDistance <= 1f)
        {
            ...
            //trigger should show shop etc

        }
    }


    public void Approach(GameObject hittransformgameObject)
    {
    ...
    void MovePlayerTowardsStatue(GameObject hittransformgameObject)
    {
        Approach(hittransformgameObject);
    }

    void CheckDistanceAndPickupOrMoveTo(GameObject hittransformgameObject)
    {
        destinationDistance = Vector3.Distance(destinationPosition, PlayerTransform.position);
        ...
        else
        {
            ...
            PlayerShouldPickup(hittransformgameObject);

        }
    }
    void CheckDistanceAndWorshipOrMove(GameObject hittransformgameObject)
    {
        destinationDistance = Vector3.Distance(destinationPosition, PlayerTransform.position);
    ...
    void PlayerShouldPickup(GameObject hittransformgameObject)
    {
        ...
        foreach (RaycastHit hit in hits)
        {
            ...
            if (hit.transform.name == hittransformgameObject.transform.name )
            {
    ...
    void CheckDistanceAndTalkOrMoveToNPC(GameObject hittransformgameObject)
    {
        destinationDistance = Vector3.Distance(destinationPosition, PlayerTransform.position);
    ...
    void CheckDistanceAndPickupOrMove(GameObject hittransformgameObject)
    {
        destinationDistance = Vector3.Distance(destinationPosition, PlayerTransform.position);
    ...
    void PlayerClicksLeftMouseButtonOnPotion(GameObject hittransformgameObject)
    {
        //ApproachAndPickUp

        CheckDistanceAndPickupOrMove(hittransformgameObject);

    }

    void PlayerClicksLeftMouseButtonOnRing(GameObject hittransformgameObject)
    {
        switch (hittransformgameObject.transform.name)
        {
            case "Shiva Shakti yantra ring":
                FacePlayerTowards(hittransformgameObject);

                CheckDistanceAndPickupOrMoveTo(hittransformgameObject);

                break;
        }
    }

    void PlayerClicksLeftMouseButtonOnAdria(GameObject hittransformgameObject)
    {
        CheckDistanceAndTalkOrMoveToNPC(hittransformgameObject);
    }
    void PlayerClicksLeftMouseButtonOnGanesha(GameObject hittransformgameObject)
    {
        FacePlayerTowards( hittransformgameObject);
        CheckDistanceAndWorshipOrMove(hittransformgameObject);
    }
    void PlayerClicksLeftMouseButtonOnKrishna(GameObject hittransformgameObject)
    {
        FacePlayerTowards(hittransformgameObject);
        CheckDistanceAndWorshipOrMove(hittransformgameObject);
    ...
    void FacePlayerTowards(GameObject hittransformgameObject)
    {        
            PlayerTransform.LookAt(hittransformgameObject.transform.position); //destinationPosition);//hit.transform.position);       
    }
   ...
   //         PlayerAnimation["Attack"].wrapMode = WrapMode.Once;
		...
		//	Debug.Log("Checking Object Under Mouse Cursor. It is: " + hit.transform.name);
			...
			} else if (ObjectUnderCursor == "Ganesha")
			{
				...
				if (Input.GetMouseButtonDown (0)) {
					...
					PlayerClicksLeftMouseButtonOnGanesha (hit.transform.gameObject);
				}
				// Moves the player if the mouse button is hold down
				else if (Input.GetMouseButton (0)) {
                    ApproachAndWorship = true;
                    ...
                    FacePlayerTowards(hit.transform.gameObject);
                } else {
					StatueTransform = hit.transform;
				}
			}
			else if (ObjectUnderCursor == "Krishna")
			{
				...
				if (Input.GetMouseButtonDown (0)) {
					...
					PlayerClicksLeftMouseButtonOnKrishna (hit.transform.gameObject);
				}
				// Moves the player if the mouse button is hold down
				else if (Input.GetMouseButton (0)) {
                    ApproachAndWorship = true;
			...
			else if (ObjectUnderCursor == "Potion")
			{
				...
				if (Input.GetMouseButtonDown (0)) {
					...
					//ApproachAndAttack = false;
					//IsAttacking = false;
					PlayerClicksLeftMouseButtonOnPotion (hit.transform.gameObject);
				}
				// Moves the player if the mouse button is hold down
				else if (Input.GetMouseButton (0)) {					
					destinationPosition =	hit.transform.position;
            ...
            else if (ObjectUnderCursor == "Pepin")
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    PlayerClicksOnGameObject(hit.transform.gameObject);
                }
            }
            ...
            else if (ObjectUnderCursor == "Griswold")
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    //PlayerClicksLeftMouseButtonOnGriswold();
            ...
            else if (ObjectUnderCursor == "Adria")
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    //ApproachAndAttack = false;
                    //IsAttacking = false;
                    PlayerClicksLeftMouseButtonOnAdria(hit.transform.gameObject);
                }
            }
            else if (ObjectUnderCursor == "Shiva Shakti yantra ring")
            {
                ...
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    //ApproachAndAttack = false;
                    //IsAttacking = false;
                    PlayerClicksLeftMouseButtonOnRing(hit.transform.gameObject);
                }
                // Moves the player if the mouse button is hold down
                else if (Input.GetMouseButton(0))
                {
    ...
    //void PlayerClicksLeftMouseButtonOnGriswold()
    //{
    //    CheckDistanceAndTalkOrMoveToNPC("Griswold");
    //}
